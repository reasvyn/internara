...
it('SE5Q9-FR-M4: fromArray() resolves camelCase keys', function () {
...
it('SE5Q9-FR-M4: fromArray() falls back to snake_case keys', function () {
    $data = TestData::fromArray(['name' => 'Adit', 'email' => 'adit@example.com']);

    expect($data->name)->toBe('Adit');
});

it('SE5Q9-FR-M4: fromArray() applies constructor defaults for omitted optional params', function () {
    $data = TestData::fromArray(['name' => 'Adit']);

    expect($data->email)->toBe('default@example.com');
});

it('SE5Q9-FR-M4: fromArray() throws when a required param is missing', function () {
...
it('SE5Q9-FR-M4: toArray() serializes the DTO and jsonSerialize() matches it', function () {
    $data = TestData::fromArray(['name' => 'Adit']);
...
it('SE5Q9-FR-M4: merge() overlays values and returns the same DTO type', function () {
...
it('SE5Q9-FR-M4: only() and except() select and drop keys', function () {
    $data = TestData::fromArray(['name' => 'Adit', 'email' => 'adit@example.com']);
