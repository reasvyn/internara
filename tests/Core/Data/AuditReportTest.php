...
it('FR-D2: passed() is true when no check has failed', function () {
...
it('FR-D2: passed() is false when a check has failed', function () {
    $report = new AuditReport([
        auditCheck(AuditCategory::REQUIREMENTS, AuditStatus::PASS),
...
it('FR-D2: forCategory() filters checks by category', function () {
    $report = new AuditReport([
        auditCheck(AuditCategory::REQUIREMENTS, AuditStatus::PASS),
        auditCheck(AuditCategory::DATABASE, AuditStatus::FAIL),
        auditCheck(AuditCategory::DATABASE, AuditStatus::WARN),
