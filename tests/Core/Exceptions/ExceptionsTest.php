...
it('FR-EH1/FR-EH5: AppException is the abstract application root with a statusCode contract', function () {
...
it('FR-EH2: ModuleException is the abstract business-rule root', function () {
    expect((new ReflectionClass(ModuleException::class))->isAbstract())->toBeTrue();
});

it('FR-EH3: ModuleException does not extend AppException (independent trees)', function () {
    expect((new ReflectionClass(ModuleException::class))->isSubclassOf(AppException::class))->toBeFalse();
});

it('FR-EH6: RejectedException extends ModuleException with status 400', function () {
...
it('FR-EH7: ValidationFailedException extends ActionException with status 422', function () {
...
it('FR-E5: UnauthorizedException extends PresentationException with status 403', function () {
...
it('FR-EH9: InfrastructureException defaults to status 500 and is not user-facing', function () {
...
it('FR-E7: HasExceptionContext provides hint, context, CLI output and reporting', function () {
