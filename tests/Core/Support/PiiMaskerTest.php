...
it('89SRA-FR-PM2: maskArray() fully masks sensitive keys regardless of case', function () {
...
it('89SRA-FR-PM1: maskArray() recursively masks nested arrays', function () {
...
it('89SRA-FR-PM3: maskValue() partially masks emails preserving the first characters and domain', function () {
...
it('89SRA-FR-PM4: maskValue() partially masks phone numbers preserving the last 4 digits', function () {
    expect(PiiMasker::maskValue('phone', '081234567890'))->toBe('********7890');
});

it('89SRA-FR-PM5: maskValue() partially masks names preserving the first initial and last name', function () {
    expect(PiiMasker::maskValue('name', 'John Smith'))->toBe('J. Smith');
    expect(PiiMasker::maskValue('name', 'Jane'))->toBe('J***');
});

it('89SRA-FR-PM6: maskIp() masks IPv4 addresses keeping the first two octets', function () {
    expect(PiiMasker::maskIp('192.168.10.5'))->toBe('192.168.***.***');
});

it('89SRA-FR-PM7: maskIp() masks IPv6 addresses keeping only the first segment', function () {
    expect(PiiMasker::maskIp('2001:db8:85a3::8a2e'))->toBe('2001::****');
});

it('89SRA-FR-PM8: maskUserAgent() truncates to 50 characters with an ellipsis', function () {
    $ua = str_repeat('a', 60);
...
it('89SRA-FR-PM9: all required sensitive keys resolve to a full mask', function () {
...
it('89SRA-FR-PM1: maskValue() leaves non-sensitive keys untouched', function () {
