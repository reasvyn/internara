...
it('FR-SUP7: hexToRgb() converts a hex color to RGB channels', function () {
...
it('FR-SUP7: rgbToHex() converts RGB channels to a hex color', function () {
    expect(Color::rgbToHex(255, 255, 255))->toBe('#ffffff');
    expect(Color::rgbToHex(255, 0, 128))->toBe('#ff0080');
});

it('FR-SUP7: relativeLuminance() is 1.0 for white and 0.0 for black', function () {
    expect(Color::relativeLuminance('#ffffff'))->toBe(1.0);
    expect(Color::relativeLuminance('#000000'))->toBe(0.0);
});

it('FR-SUP7: contrastColor() picks a dark tone for light backgrounds and a light tone for dark backgrounds', function () {
    expect(Color::contrastColor('#ffffff'))->toBe('#1a1a1a');
    expect(Color::contrastColor('#000000'))->toBe('#f0f0f0');
});

it('FR-SUP7: lighten() and darken() adjust channel brightness', function () {
    expect(Color::lighten('#000000', 10))->toBe('#1a1a1a');
    expect(Color::darken('#ffffff', 50))->toBe('#7f7f7f');
});

it('FR-SUP7: isValid() accepts six-digit hex with optional hash only', function () {
...
it('FR-SUP7: computeBaseShades() builds daisyUI base shades from a light color', function () {
...
it('FR-SUP7: computeDarkShades() darkens a light color for dark mode', function () {
