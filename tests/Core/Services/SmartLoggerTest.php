...
test('89SRA-FR-SL1: SmartLogger is only reachable through its static factories', function () {
    expect((new ReflectionClass(SmartLogger::class))->getConstructor()->isPrivate())->toBeTrue();
});

test('89SRA-FR-SL2/FR-DC1: system channel maps success to info and error to error', function () {
    $logs = captureLogs();
...
test('89SRA-FR-SL3: for() and about() attach the causer and subject context', function () {
    $logs = captureLogs();
...
test('89SRA-FR-SL6: PII masking scrubs sensitive payload keys in the system log', function () {
    $logs = captureLogs();
...
test('89SRA-FR-DC5: channel() tags the system log context', function () {
    $logs = captureLogs();
...
test('89SRA-FR-DC6: systemOnly() writes to the system channel and never to the activity log', function () {
    $logs = captureLogs();
...
test('89SRA-FR-DC6: activityOnly() writes an activity row even without a causer', function () {
    SmartLogger::info('test message')->activityOnly()->save();

    expect(DB::table('activity_log')->where('description', 'test message')->count())->toBe(1);
});

test('89SRA-FR-DC3: both() writes to the system and activity channels with a causer', function () {
    $logs = captureLogs();
    $user = User::factory()->create();
...
test('89SRA-FR-EI1: event() accepts a dot-notation event name and tags the system log', function () {
    $logs = captureLogs();
...
test('89SRA-FR-EI2: save() dispatches a BaseEvent through the event dispatcher', function () {
    Event::fake();
    $subject = User::factory()->create();
...
test('89SRA-FR-EI3/FR-EI4: event payload is merged and PII-masked before writing', function () {
    $logs = captureLogs();
    $subject = User::factory()->create();
...
test('89SRA-FR-TR1/FR-TR2: translation context resolves for the active and alternate locale', function () {
    $logs = captureLogs();
    App::setLocale('en');
