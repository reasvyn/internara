...
it('FR-EV1: every event class in the codebase extends BaseEvent', function () {
...
it('FR-EV5/NFR-EV5: every config/event.php mapping is boot-valid', function () {
...
it('FR-EV6: every registered event has at least one listener', function () {
    foreach (config('event.listen') as $event => $listeners) {
        expect($listeners)->not->toBeEmpty("Event {$event} must have a listener");
    }
});

it('FR-EV6: every config-registered listener is wired to a real event class', function () {
    $mappings = config('event.listen');
