...
class CodeStabilityTest extends TestCase
{
    ...
    public function test_no_hardcoded_paths_in_app(): void
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (preg_match('/mysql_connect|mysqli_connect/', $content)) {
                $violations[] = $relativePath . ': Direct database connection detected (use Laravel DB facade)';
            }
        }
    ...
    public function test_no_raw_sql_in_models(): void
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (preg_match('/DB::raw\s*\(/', $content) || preg_match('/\->select\s*\(DB::raw/', $content)) {
                $violations[] = $relativePath . ': Raw SQL query detected (use Eloquent or parameterized queries)';
            }
        }
    ...
    public function test_no_silent_catch_blocks(): void
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (preg_match('/catch\s*\([^)]+\)\s*\{\s*(?:\/\/.*\s*)*\s*\}/', $content)) {
                $violations[] = $relativePath . ': Empty catch block detected (handle or log the error)';
            }
        }
        
        $this->assertEmpty($violations, "Silent catch blocks found:\n" . implode("\n", $violations));
    }
    ...
    public function test_env_usage_in_config_only(): void
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (str_starts_with($relativePath, 'config/') || str_starts_with($relativePath, '.env')) {
                continue;
            }
            ...
            if (preg_match('/env\s*\(/', $content)) {
                $violations[] = $relativePath . ': Direct env() call detected (use config() instead)';
            }
        }
        
        $this->assertEmpty($violations, "env() usage outside config found:\n" . implode("\n", $violations));
    }
    ...
    public function test_no_deprecated_laravel_methods(): void
    {
        $deprecatedPatterns = [
            '/->whereNotExists\(/',
            '/->pluck\([^,]+\)/', // pluck with single argument is deprecated in some contexts
        ];
