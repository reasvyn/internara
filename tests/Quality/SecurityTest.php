...
class SecurityTest extends TestCase
{
    ...
    public function test_actions_validate_input(): void
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (preg_match('/function\s+execute\s*\([^)]*array\s+\$/', $content)) {
                ...
                $hasValidation =
                    preg_match('/\$this->validate\s*\(/', $content) ||
    ...
    public function test_controllers_check_authorization(): void
    {
        ...
        foreach ($iterator as $file) {
            ...
            $hasDataModificationMethods = preg_match(
                '/function\s+(store|update|destroy|delete|create)\s*\(/',
                $content,
            );
            ...
            if ($hasDataModificationMethods) {
                ...
                $hasAuthorization =
                    preg_match('/\$this->authorize\(/', $content) ||
