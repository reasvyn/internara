...
test('mathematical weighted verification: composite score matches program config', function () {
    ...
    $attendanceScore = 100.00; // 100% presence
    $journalScore = 80.00;     // 80% journal submission
    $rubricScore = 90.00;      // qualitative evaluation
...
test('participation capping audit: scores are capped at 100.00', function () {
    ...
    $capped = min($percentage, 100.00);
    
    expect($capped)->toBe(100.00);
});
