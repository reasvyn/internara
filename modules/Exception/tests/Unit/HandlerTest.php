...
describe('Exception Handler Transformation (Genesis Contract)', function () {
    test('it transforms Eloquent ModelNotFoundException into domain RecordNotFoundException', function () {
        $infrastructureException = (new EloquentModelNotFound)->setModel('Modules\User\Models\User', ['uuid-123']);
        
        $transformed = Handler::map($infrastructureException);
        
        expect($transformed)->toBeInstanceOf(RecordNotFoundException::class)
            ->and($transformed->uuid)->toBe('uuid-123')
            ->and($transformed->module)->toBe('user');
    });
