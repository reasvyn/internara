...
test('requirement guard audit: it rejects approval if mandatory requirements are missing', function () {
    ...
    expect(fn() => app(RegistrationService::class)->approve($registration->id))
        ->toThrow(\Modules\Exception\AppException::class, 'internship::exceptions.mandatory_requirements_not_met');
});

test('event governance audit: it dispatches InternshipRegistered with lightweight payload', function () {
    Event::fake();
    
    $program = app(\Modules\Internship\Services\Contracts\InternshipService::class)->factory()->create();
    $placement = app(\Modules\Internship\Services\Contracts\InternshipPlacementService::class)->factory()->create(['internship_id' => $program->id]);
    ...
    Event::assertDispatched(function (InternshipRegistered $event) use ($registration) {
