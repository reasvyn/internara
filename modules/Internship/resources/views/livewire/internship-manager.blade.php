<div>
    <x-ui::main title="{{ __('internship::ui.program_title') }}" subtitle="{{ __('internship::ui.program_subtitle') }}">
        <x-slot:actions>
            <x-ui::button label="{{ __('internship::ui.add_program') }}" icon="tabler.plus" class="btn-primary" wire:click="add" />
        </x-slot:actions>
            ...
            <x-ui::table :headers="[
                ...
                @scope('actions', $program)
                    <div class="flex gap-2">
                        <x-ui::button icon="tabler.edit" class="btn-ghost btn-sm text-info" wire:click="edit('{{ $program->id }}')" tooltip="{{ __('shared::ui.edit') }}" />
                        <x-ui::button icon="tabler.trash" class="btn-ghost btn-sm text-error" wire:click="discard('{{ $program->id }}')" tooltip="{{ __('shared::ui.delete') }}" />
                    </div>
                @endscope
            </x-ui::table>
        </x-ui::card>
