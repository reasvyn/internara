...
class AdminService extends EloquentQuery implements Contract
{
    ...
    {
        return [
            'total_admins' => $this->model->newQuery()->role([Role::ADMIN->value, Role::SUPER_ADMIN->value])->count(),
            'active_admins' => $this->model->newQuery()->whereNotNull('email_verified_at')->role([Role::ADMIN->value, Role::SUPER_ADMIN->value])->count(),
    ...
    {
        return app(\Modules\Log\Services\Contracts\AuditServiceInterface::class)
