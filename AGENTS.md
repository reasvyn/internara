...
This project has domain-specific skills available in `**/skills/**`. You MUST activate the relevant
skill whenever you work in that domain—don't wait until you're stuck.
...
- Do not create verification scripts or tinker when tests cover that functionality and prove they
  work. Unit and feature tests are more important.
...
- Use `get-absolute-url` to resolve the correct scheme, domain, and port for project URLs. Always
...
- CRITICAL: ALWAYS use `search-docs` tool for version-specific Laravel documentation and updated
  code examples.
...
- The `app/Console/Kernel.php` file no longer exists; use `bootstrap/app.php` or
  `routes/console.php` for console configuration.
- Console commands in `app/Console/Commands/` are automatically available and do not require manual
  registration.
...
- `upsert()` now requires a non-empty `uniqueBy` parameter (throws `InvalidArgumentException` if
  empty).
...
- Model instances cannot be created during the boot cycle (throws `LogicException`).

## Cache

- `Cache::touch($key, $seconds)` extends TTL without re-fetching or re-storing the value.
- `serializable_classes` configuration hardens cache unserialization (default: `false`).
- Cache prefix uses hyphenated suffix: `app-name-cache-`.

## Security

- CSRF middleware renamed from `VerifyCsrfToken` to `PreventRequestForgery`.
- Request forgery protection now includes origin verification via `Sec-Fetch-Site` header.
