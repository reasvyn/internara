        ...
        <x-ui::card wire:key="{{ $this->getEventPrefix() }}-card" class="overflow-hidden border-none shadow-xl bg-base-100/60 ring-1 ring-base-content/5 rounded-[2.5rem]">
            <div class="table-enterprise relative">
                <div 
                    ...
                    x-show="hasItems && (filteredCount > 0 || !search)"
                >
                    <x-mary-table 
                    ...
                    >
                        ...
                        @if(!isset($tableCells) && !isset($rowActions))
                            @scope('cell_actions', $record)
                                <div class="flex items-center justify-end gap-1 px-2">
                                    @php
                                        $rowActionsForRecord = $record->actions ?? [];
                                    @endphp
                                    @forelse($rowActionsForRecord as $action)
                                        <x-ui::button 
                                            icon="{{ $action['icon'] }}" 
                                            variant="{{ $action['variant'] ?? 'tertiary' }}" 
                                            class="{{ $action['class'] ?? 'btn-circle btn-xs' }}"
                ...
                {{-- Instant Empty State Feedback (Client-side: has items but search finds none) --}}
                <div 
                    x-show="search && filteredCount === 0 && hasItems" 
                    x-cloak 
                ...
                {{-- Server-side Empty State (When table is actually empty - no records at all) --}}
                <div 
                    x-show="!search && !hasItems" 
                    x-cloak 
                    class="py-20 text-center"
                >
                    <div class="relative inline-flex mb-6">
