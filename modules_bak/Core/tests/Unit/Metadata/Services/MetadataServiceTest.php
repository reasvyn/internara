...
describe('MetadataService', function () {
    ...
    test('it returns author info', function () {
        ...
        expect($author)->toHaveKey('name', 'Reas Vyn')->toHaveKey('email', 'dev@internara.test');
    });
    ...
    test('it returns brand name from app_info', function () {
        ...
        $brandName = $this->service->getBrandName();
        expect($brandName)->toBe('Internara');
    });

    test('it returns brand name defaults to Internara when empty', function () {
        $data = ['name' => 'Internara'];
        File::put($this->path, json_encode($data));

        // getBrandName() falls back to getAppName() which returns 'Internara'
        $brandName = $this->service->getBrandName();
        expect($brandName)->toBe('Internara');
    });
});
