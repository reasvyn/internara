...
namespace App\Domain\User\Aggregates\ActivationToken\Livewire;

use App\Domain\User\Aggregates\ActivationToken\Models\ActivationToken;
...
class ActivateAccount extends Component
{
    ...
    public function activate(): void
    {
        ...
        $token = ActivationToken::where('user_id', $user->id)
            ->where('token_type', 'activation')
            ->where('expires_at', '>', now())
            ->first();

        if (! $token || ! Hash::check($this->code, $token->token)) {
            RateLimiter::hit($throttleKey, 300);

            if ($token) {
                $token->increment('attempts');
                $token->update(['last_attempt_at' => now()]);
            }
    ...
    {
        return view('user.activation-token.activate-account');
    }
}
