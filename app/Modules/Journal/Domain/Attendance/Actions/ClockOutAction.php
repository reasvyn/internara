...
use App\Modules\Journal\Domain\Attendance\Events\AttendanceClockOut;
...
final class ClockOutAction extends BaseCommandAction
{
    public function execute(ClockOutData $data): Attendance
    {
        return $this->transaction(function () use ($data) {
            ...
            if (! $log) {
                throw new RejectedException(__('journal.attendance.must_clock_in_first'));
            }

            if ($log->clock_out) {
                throw new RejectedException(__('journal.attendance.already_clocked_out'));
            }
