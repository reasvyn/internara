...
namespace App\Modules\Journal\Domain\SupervisionLog\Livewire;
...
class StudentLogManager extends BaseRecordManager
{
    ...
    {
        return [
            ['index' => 'date', 'label' => __('journal.date'), 'sortable' => true],
            ['index' => 'topic', 'label' => __('journal.topic')],
            ['index' => 'status', 'label' => __('journal.status')],
            ['index' => 'supervisor_feedback', 'label' => __('journal.feedback')],
    ...
    public function save(CreateLogAction $action): void
    {
        ...
        if (! $registration) {
            $this->toast()->error(__('journal.no_active_registration'))->send();
        ...
        $this->toast()->success(__('journal.log_created'))->send();
    ...
    public function confirmAction(DeleteLogAction $action): void
    {
        ...
        try {
            ...
            $action->execute($log);
            $this->toast()->success(__('journal.log_deleted'))->send();
    ...
    {
        return view('journal.supervision-log.student-log-manager');
    }
}
