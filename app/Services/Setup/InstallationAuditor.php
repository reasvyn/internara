...
class InstallationAuditor
{
    ...
    public function audit(): array
    {
        ...
        $checks = [...$checks, ...$this->checkRequiredExtensions()];
        $checks = [...$checks, ...$this->checkRecommendedExtensions()];
        $checks = [...$checks, ...$this->checkWritableDirectories()];
        $checks[] = $this->checkDatabaseConnection();
        $checks[] = $this->checkAppKey();

        return [
            'passed' => collect($checks)->every(fn ($c) => $c['status'] !== 'fail'),
            'checks' => $checks,
        ];
    }
