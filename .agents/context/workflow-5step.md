...
Since 2026-08-21, the canonical agent workflow is **5 steps** (SSOT: `AGENTS.md §Agent Workflow` + `AGENTS.md#Agent Workflow`):
...
| **2. Plan** | Context, approach, design **before** touching code | Read module docs + `docs/guides/arch/*.md` + full file content of every file you may touch; survey `tools/` (Automation-First); consider 2+ approaches; decide Action triad (`Command`/`Read`/`Process`), Entity (`final readonly` + `fromModel()`), DTO (`BaseData`), Model (`#[Fillable]`), error `RejectedException` (C8), cache registry (C4) | 2+ approaches + chosen design + test/doc plan |
| **3. Implement** | Surgical execution + docs | Smallest change, preserve unrelated code; `declare(strict_types=1)` (D1), no `dd()` (D2), `__()` (D3), no raw `$request->all()` (D5), FK `onDelete` (D6), no Model mutations in Livewire (C1), constructor injection (C2); update PHPDoc in same step; DRY dedup on sight | Code + docs/PHPDoc in sync, `git status` only intended files |
| **4. Verify** | Quality gates, **batched once** | `git status`+`git diff` (Edit Policy), `vendor/bin/pint --dirty --test`, `npx prettier --check`, `php artisan test --compact --filter` / `vendor/bin/pest --testsuite`, `python3 tools/scan_*.py` batch; full suite **on-demand only** (~2GB, 10+ min) | Change-type matrix (AGENTS.md#Verification Strategy) passes |
...
- `AGENTS.md §Agent Workflow` — canonical 5-step definition (SSOT)
- `AGENTS.md#Agent Workflow` + `#Phase Classification` + `#Size Triage` + `#Edit Policy` + `#Verification Strategy`
- `.agents/rules/instruction-ordering.md` — impact-to-effort scoring
- `.agents/rules/key-rules.md` — 6 non-negotiable rules
