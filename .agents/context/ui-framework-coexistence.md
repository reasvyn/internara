...
|---|---|---|
| `robsontenorio/mary` | **removed** from `composer.json`/`composer.lock`; `config/mary.php` + `app/Modules/Core/Support/Spotlight.php` deleted; `extra.laravel.dont-discover` now `[]` | TallstackUI `x-ts-*` |
...
* The old Livewire `ThemeSwitcher` (`app/Modules/Settings/Livewire/ThemeSwitcher.php`) + its view + `fl-dark` class + MutationObserver (all PHPFlasher-era) were removed; replaced by `core::ui.theme-switch` wrapping `<x-theme-switch>`.
