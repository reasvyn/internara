...
6. **Dead code** — Unused DTOs, unregistered observers, events without listeners. Verify with `python3 tools/scan_dead_code/cli.py --module {Module}`.
...
2. `python3 tools/scan_violations/cli.py --module {Module}` + `scan_class_contracts.py --module {Module}` — baseline the debt
...
| Diagnose unknown failure | Check `README.md#Project Status` for tier, then `python3 tools/scan_dead_code/cli.py` + `git blame` for that module |
...
- `tools/scan_violations/cli.py`, `scan_class_contracts.py`, `scan_dead_code.py` — health scanners
