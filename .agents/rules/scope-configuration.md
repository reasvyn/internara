...
| **Agent guides, context & memory** | `AGENTS.md`, `.agents/skills/*/SKILL.md`, `.agents/context/`, `.agents/memory/` against the specs they reference (audit reports live in GitHub Issues, not `.agents/plans/`) | `spec-audit --guides` |
...
| **Documentation** | Docs reflect the spec and code: Quick Reference paths exist, cross-refs are valid, `docs/guides/arch/*` and `docs/refs/modules/*` match the spec's contracts, and **agent guides & skills** stay consistent with the spec | `cross-refs`, `paths`, `guides`, §6 completeness + doc-to-code sync |
