...
```bash
python3 -m py_compile tools/{name}.py                # syntax gate
python3 tools/{name}.py --module {Module} --strict   # scoped run + strict exit
python3 tools/{name}.py --json | jq '.findings[0]'   # finding field contract
```
