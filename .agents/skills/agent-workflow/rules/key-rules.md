...
**Why it matters:** `agent-workflow` is the single source of truth for the 9-step pipeline, the
4-phase mapping, size triage, narration discipline, and verification strategy. Loading it first
guarantees the agent operates on current workflow facts even when other skills or files restate them
in stale form. Skipping it means task-specific skills (which assume it) are interpreted without
...
9-step or 4-phase pipeline, generic Construct/Verify steps, or the AGENTS.md 9-step mapping.
...
skill for the canonical 9-step pipeline / 4-phase model — this skill adds {X} — nothing else."
...
**Verification:** `python3 scripts/scan_skills.py` reports no `SKILL_NO_DUP_WORKFLOW` findings.
...
**How to apply:** In Construct, locate the spec (foundation, module, or feature) via
`docs/specs/index.md`, read its FR/NFR/UC IDs, and drive definition, design, and tests from it. If
no requirement exists, write it into the spec first — spec-first, never fix-first. If spec and code
disagree, align code to spec ("fix code, assert spec"); if the spec is demonstrably wrong, amend the
spec with a recorded decision first.
...
**What it enforces:** The 9 pipeline steps are internal reasoning and must not be narrated, restated,
...
**How to apply:** Group edits, then run the change-type matrix in `AGENTS.md` §Verification Strategy
(translation keys → pint + tinker echo; blade/css/js → `npm run build`; refactor → targeted filter;
new business logic → full suite once). Review `git status` + `git diff` before/after every change.
