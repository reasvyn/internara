...
- If the task was classified **L**, present the proposed session plan (each session = one deliverable
  unit with its own verify + report) and get user approval before execution
- If the work is repetitive, batch, or pattern-based, note the reusable scripts/devtools to use
  (Automation-First — check `scripts/` and the Automation Scripts section below first)
...
1. **CONSTRUCT** — Read relevant docs and existing code; verify paths and signatures; consider
   multiple approaches
2. **EVALUATE** — Does it match requirements (FR/NFR/UC from the governing spec)? Respect layer
   boundaries? Do ONE thing?
3. **VERIFY** — Lint + static analysis + tests pass; no debug calls; `__()` for strings
4. **DECIDE** — Accept / Revise / Split / Escalate / Defer
   - **Split** when: task classified **L** (Size Triage) or scope grew beyond one session — inform
     the user, propose a session plan, never push through in one pass
   - **Escalate** when: the decision changes scope or architecture, or a governing spec is missing
     or ambiguous — surface it to the user rather than guessing
...
| C7  | DTO required for 3+ parameters in Command/Process Actions               |
...
| D4  | Models use `#[Fillable]` attribute, not `$fillable`                     |
...
| `scan_violations.py` | C1-C8, D1-D6 invariant violations | `python3 scripts/scan_violations.py` |
| `scan_class_contracts.py` | Action/Entity/DTO/Model/Enum class contracts | `python3 scripts/scan_class_contracts.py` |
...
| `scan_conventions.py` | strict_types, Fillable, debug calls | `python3 scripts/scan_conventions.py` |
| `scan_doc_links.py` | Broken links in docs | `python3 scripts/scan_doc_links.py` |
| `scan_tests.py` | Per-module test results | `python3 scripts/scan_tests.py` |
| `scan_skills.py` | SKILL.md meta-framework consistency | `python3 scripts/scan_skills.py` |
| `scan_issues.py` | GitHub issues by module/severity | `python3 scripts/scan_issues.py` |
| `scan_dead_code.py` | Dead code detection | `python3 scripts/scan_dead_code.py` |

Output: `scripts/outputs/{timestamp}-{description}.json`.

**Automation-First:** before doing manual or repeated work, check `scripts/` and this table for an
existing scanner or helper. Never redo by hand what a script does. If a recurring pattern has no
script, load `script-automation` to add one.
