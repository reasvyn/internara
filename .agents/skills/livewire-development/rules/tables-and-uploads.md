...
a mandated mechanism — maryUI's `x-mary-table` (with `BaseRecordManager` for CRUD) and
...
## Tables Use maryUI `x-mary-table`

**What it enforces:** List views use maryUI's `x-mary-table` component with sorting and pagination.
...
**Why it matters:** maryUI's table ships sorting, pagination, selection, and the accessible header
semantics this project requires (see `rules/accessibility.md`). Hand-rolling an HTML table duplicates
...
CRUD tables. For custom lists, render `x-mary-table` with defined columns wired to a paginated,
...
**Verification:** The view uses `x-mary-table` (or `BaseRecordManager`); sorting, pagination, and
