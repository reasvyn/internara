...
Components live in `app/{Module}/Livewire/` and are auto-discovered by AppServiceProvider. The alias pattern is `{kebab-module}.{kebab-class-name}`. Views are at `resources/views/{domain}/{component-name}.blade.php`. CRUD table components extend `BaseRecordManager` which provides pagination, search, sort, and selection.
