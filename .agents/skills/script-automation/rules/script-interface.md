...
Every script in `scripts/` exposes the same command-line interface so agents and CI can drive any
...
**Why it matters:** a predictable `scripts/` directory means the Automation-First discipline works —
...
**How to apply:** scripts are named `scan_{name}.py` for scanners; test files go in `scripts/tests/`;
...
Default output path: `scripts/outputs/{YYYYMMDDHHSSMS}-{scan_name}.json`
