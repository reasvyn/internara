...
```bash
python3 scripts/{name}.py --json | jq '.summary'         # shape smoke test
python3 scripts/{name}.py --json | jq '.findings[0]'     # field contract test
python3 scripts/{name}.py --json | python3 -c "import json,sys; d=json.load(sys.stdin); assert sorted(d) == ['scan_version','scan_name','scan_type','module','timestamp','execution_time_ms','summary','findings','metadata']"
```
