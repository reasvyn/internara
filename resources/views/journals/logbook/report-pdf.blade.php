...
<body>
    ...
    <div class="header-block">
        <table>
            <tr>
                <td>{{ __('logbook.report_student') }}</td>
                <td>{{ $student->name }}</td>
            </tr>
            @if ($company)
                <tr>
                    <td>{{ __('logbook.report_company') }}</td>
                    <td>{{ $company->name }}</td>
                </tr>
            @endif
            <tr>
                <td>{{ __('logbook.report_period') }}</td>
                <td>
                    {{ $registration->start_date?->format('d M Y') }} — {{ $registration->end_date?->format('d M Y') }}
                </td>
            </tr>
            <tr>
                <td>{{ __('logbook.report_entries') }}</td>
                <td>{{ $entries->count() }} {{ __('logbook.report_entries_count') }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="footer">{{ __('logbook.report_generated') }}: {{ now()->format('d M Y H:i') }}</div>
</body>
</html>
