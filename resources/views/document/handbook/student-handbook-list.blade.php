                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-2">
                        @if($entity->isNewerThan($lastAck))
                            <x-mary-button :label="__('handbook.acknowledge')" icon="o-check" class="btn-primary btn-sm" wire:click="acknowledge('{{ $handbook->id }}')" />
                        @else
                            <x-mary-badge :value="__('common.actions.done')" class="badge-success badge-sm" />
                        @endif

                        <x-mary-button :label="__('handbook.download')" icon="o-arrow-down-tray" class="btn-ghost btn-sm" wire:click="download('{{ $handbook->id }}')" />
                    </div>
                </div>
            </x-mary-card>
        @empty
            <div class="col-span-full">
                <x-mary-alert :title="__('handbook.no_handbooks')" icon="o-information-circle" class="bg-base-200" />
            </div>
        @endforelse
    </div>
</div>
