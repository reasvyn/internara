<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        @if($registration)
            <x-shared::widgets.stat-card :title="__('dashboard.student.company')" :value="$registration->placement->company->name" icon="o-building-office" color="text-primary" />
            <x-shared::widgets.stat-card :title="__('dashboard.student.position')" :value="$registration->placement->name" icon="o-briefcase" color="text-secondary" />
            <x-shared::widgets.stat-card :title="__('dashboard.student.batch')" :value="$registration->internship->name" icon="o-academic-cap" color="text-accent" />
        @else
            <div class="sm:col-span-3">
                <x-mary-card class="bg-base-100 border border-base-content/10">
                    <x-shared::widgets.empty-state icon="o-shield-exclamation" :title="__('dashboard.student.no_registration')" :description="__('dashboard.student.no_registration_hint')" />
                </x-mary-card>
            </div>
        @endif
    </div>
