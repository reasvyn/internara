...
>
    ...
    <x-slot:stats>
        <x-core::widgets.stat-card icon="o-users" :title="__('user.manager.stats_total')" :value="$this->stats['total']" color="text-primary" />
        <x-core::widgets.stat-card icon="o-shield-check" :title="__('user.manager.stats_admins')" :value="$this->stats['admins']" color="text-secondary" />
        <x-core::widgets.stat-card icon="o-check-badge" :title="__('user.manager.stats_active')" :value="$this->stats['active']" color="text-success" />
        <x-core::widgets.stat-card icon="o-clock" :title="__('user.manager.stats_pending')" :value="$this->stats['pending']" color="text-warning" />
    </x-slot:stats>
    ...
    <x-slot:modal>
        <x-mary-modal wire:model="userModal" :title="$form->id ? __('user.manager.edit') : __('user.manager.new')" separator class="backdrop-blur-sm">
            <x-mary-form wire:submit="saveUser" class="space-y-5">
                ...
                <div class="bg-base-200/30 border border-base-content/10 rounded-xl p-5">
                    <p class="text-xs font-semibold uppercase tracking-wider text-base-content/50 mb-4">{{ __('user.manager.profile') }}</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-base-200/30 border border-base-content/10 rounded-xl p-5">
                    <p class="text-xs font-semibold uppercase tracking-wider text-base-content/50 mb-4">{{ __('user.fields.emergency') }}</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
