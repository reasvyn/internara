<div class="animate-in fade-in slide-in-from-bottom-8 duration-1000">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div>
            <h2 class="text-3xl font-black tracking-tightest text-base-content">{{ __('user.admin.title') }}</h2>
            <p class="text-[10px] font-black uppercase tracking-[0.3em] text-base-content/40 mt-2">{{ __('user.admin.subtitle') }}</p>
        </div>
        <x-mary-button :label="__('user.admin.new')" icon="o-plus" class="btn-primary rounded-[2rem] font-black uppercase tracking-[0.2em] text-[10px] px-8 h-12 shadow-2xl shadow-primary/30 hover:scale-[1.02] transition-transform" wire:click="create" />
    </div>
    ...
    <x-mary-modal wire:model="userModal" :title="$userData['id'] ? __('user.admin.edit') : __('user.admin.new')" class="backdrop-blur-sm" box-class="rounded-[2.5rem] p-6 border border-base-content/5 shadow-2xl">
        <div class="grid grid-cols-1 gap-6 pt-4">
            ...
            @if(!$userData['id'])
                <div class="bg-info/10 border border-info/20 text-info-content p-4 rounded-[1.5rem] flex items-start gap-4">
                    ...
                    <div>
                        <span class="font-black uppercase tracking-widest text-[9px] text-info block mb-1">Notice</span>
                        <span class="text-xs font-medium leading-relaxed">{{ __('setup.wizard.username_notice') }}</span>
                    </div>
                </div>
            @endif
        </div>
        ...
        <x-slot:actions>
            <div class="flex gap-4 pt-6 border-t border-base-content/5 w-full justify-end">
                ...
                <x-mary-button :label="__('user.admin.save')" type="submit" class="btn-primary rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-[10px] px-10 shadow-xl shadow-primary/20" wire:click="save" spinner="save" />
            </div>
        </x-slot:actions>
    </x-mary-modal>
</div>
