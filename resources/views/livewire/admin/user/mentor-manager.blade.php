<div>
    <x-mary-header title="Mentor Management" subtitle="Manage company-side internship supervisors" separator>
        <x-slot:actions>
            <x-mary-button label="Add Mentor" icon="o-plus" wire:click="createUser" class="btn-primary" />
        </x-slot:actions>
    </x-mary-header>
        ...
        <x-mary-table :headers="$headers" :rows="$users" with-pagination>
            @scope('actions', $user)
                <div class="flex gap-2">
                    ...
                    <x-mary-button icon="o-trash" wire:click="deleteUser('{{ $user->id }}')" wire:confirm="Delete this mentor?" class="btn-sm btn-ghost text-error" />
                </div>
            @endscope
        </x-mary-table>
    </x-mary-card>

    <x-mary-modal wire:model="userModal" title="{{ $userData['id'] ? 'Edit Mentor' : 'Add Mentor' }}" separator>
        <x-mary-form wire:submit="saveUser">
            ...
            <x-mary-input label="Phone Number" wire:model="userData.phone" icon="o-phone" />
