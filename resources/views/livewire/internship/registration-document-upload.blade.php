<div>
    @if($registration)
        <x-mary-card class="bg-base-100 border border-base-content/10">
            <x-slot:title>
                <span class="font-semibold">{{ __('document.upload_title') }}</span>
            </x-slot:title>
                ...
                @foreach($requirements as $req)
                    <div class="flex items-start gap-4 p-4 rounded-lg bg-base-200/30 border border-base-content/10">
                        <div class="flex-1 min-w-0">
                            ...
                            @endphp
                            @if($existing)
                                <p class="text-xs text-success mt-1">{{ __('document.uploaded') }}</p>
                            @else
    ...
    @else
        <div class="text-center py-12 text-base-content/20">
            ...
            <p class="text-sm font-medium">{{ __('document.no_registration') }}</p>
        </div>
    @endif
</div>
