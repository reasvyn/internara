<div>
    ...
    <div class="bg-base-100 border border-base-content/10 rounded-xl">
        @if($currentStep === 1)
            @include('sysadmin.setup.components.welcome-step', ['auditResults' => $audit, 'auditPassed' => $auditPassed])
        @endif
        ...
        @if($currentStep === 5)
            @include('sysadmin.setup.components.internship-step')
        @endif

        @if($currentStep === 6)
            @include('sysadmin.setup.components.finalize-step')
        @endif

        @if($currentStep === 7)
            @include('sysadmin.setup.components.complete-step')
        @endif
    </div>

    @include('sysadmin.setup.components.setup-guide')
</div>
