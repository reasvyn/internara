<div {{ $attributes->merge(['class' => 'inline-flex items-center']) }}>
    <x-ts-dropdown position="bottom-end">
        <x-slot:action>
            ...
            >
                <x-ts-icon name="flag" class="size-3.5 opacity-60" />
                <span aria-hidden="true">{{ strtoupper($locale) }}</span>
            </button>
        </x-slot:action>

        <div class="min-w-40">
            <x-ts-dropdown.items :text="__('common.language.indonesian')" icon="flag" wire:click="changeLocale('id')" />
            <x-ts-dropdown.items :text="__('common.language.english')" icon="flag" wire:click="changeLocale('en')" />
        </div>
    </x-ts-dropdown>
</div>
