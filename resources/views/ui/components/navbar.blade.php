...
>
    ...
    <div class="flex min-w-0 items-center gap-6">
        ...
        {{-- Desktop nav links --}}
        <div class="hidden items-center gap-1 lg:flex">{{ $slot }}</div>
    </div>

    {{-- Right: actions --}}
    <div class="flex items-center gap-2 sm:gap-3">
        @isset($actions)
            <div class="hidden items-center gap-2 md:flex">{{ $actions }}</div>
        @endisset

        {{-- Default actions if no slot provided --}}
        @if (! isset($actions))
            <div class="hidden items-center gap-2 md:flex">
                <x-setting::locale.theme-switch size="sm" />
    ...
    <div
        ...
        x-cloak
        id="navbar-mobile"
        class="bg-base-100 border-base-content/10 absolute top-full right-0 left-0 border-t p-4 shadow-lg lg:hidden"
