...
The application follows a dual-track observability strategy combining standard file-based logging
for debugging and Laravel Pulse for real-time system monitoring.
...
- Use appropriate levels: `debug` (development), `info` (workflow milestones), `warning` (handled
  errors), `error` (unexpected failures).
...
The system is configured to support custom Taps and Processors if advanced log filtering (e.g., PII
masking in files) is required.
