...
**Goal**: Establish the "Engine Room" (Shared & Core) to house universal concerns and RBAC infrastructure.
...
*   **New Module**: `Shared`.
*   **Scope**: Business-agnostic traits (`HasUuid`, `HasStatuses`) and the `EloquentQuery` base service.
...
- [x] **Isolated Permissions**: Authorization checks rely on standard Gates/Policies, not direct database queries.
