...
**Purpose**: Implement high-frequency activity tracking systems (Journals and Attendance) and ensure data is scoped by Academic Year for multi-year integrity.
...
**Scope**:
The system has the structural foundation (Schools, Users), but lacks operational tracking. There is no mechanism to record "what happened today" or to isolate data between different academic periods.
...
**Feature Set**:
- **Journal System**: Daily student logbooks with draft support and dual-authority approval (Mentor/Teacher).
- **Attendance Tracking**: Check-in/out system with automated late status determination.
- **Academic Year Scoping**: Global scoping trait to ensure data remains relevant to the active cohort.
...
- As a **Mentor**, I want to see if my interns are present on-site today so that I can manage their schedule.
...
**Multi-Language**:
- Status labels (Present, Late, Absent) and journal fields are localized in **Indonesian** and **English**.
...
- [x] **Secure Media**: Journal attachments are stored on private disks and accessible only via signed URLs.
