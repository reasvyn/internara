...
Admins rely on manual verification for internships and assessments, leading to bottlenecks and errors. 
There is no automated enforcement of prerequisites (e.g., "Must upload Parent Permission Letter").

**Goal**: Transform Internara into an "Active Orchestrator" that enforces administrative rules automatically.
...
- **Integration**: Pulls compliance data (Attendance % / Journal Count) from `Attendance` and `Journal` modules.
