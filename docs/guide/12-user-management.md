...
After saving, Internara generates an **account slip** with the user's initial credentials.
Download and share it with the user.
...
The User Manager supports batch creation via CSV upload. Click the menu next to **New User**,
select **Import**, and upload a file with columns: `name`, `email`, `phone`. Rows with existing
email addresses are skipped. A summary shows the result.
