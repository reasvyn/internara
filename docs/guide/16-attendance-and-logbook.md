...
You can edit a draft entry at any time. Once submitted, the entry is locked and cannot be edited. If
your mentor requests revisions, the entry status changes to **Revision Required** and you can edit
it again.
...
If you are unable to attend your internship due to illness, personal matters, or emergencies, submit
an absence request.
...
Mentors (teachers and supervisors) can view and verify attendance logs and logbook entries for their
assigned students.
...
Supervisors can add private notes to any logbook entry. These notes do not affect the entry's status
or verification state — they are informational only, visible to other supervisors and
administrators.
...
3. The system generates a PDF with all verified entries in chronological order, including supervisor
   notes (optional)
...
The system automatically monitors logbook compliance. If a student has no logbook entry for **3
consecutive days**, the mentor receives a notification. At **5 consecutive days** without an entry,
the program coordinator is also notified.
...
**Next:
[Chapter 17: Monitoring Visit & Supervision Log](17-monitoring-visit-and-supervision-log.md)**
