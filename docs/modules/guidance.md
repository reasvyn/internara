...
- **Access Controls:** Supervision logs are private. Students cannot read supervision notes written by their teachers.
- **Immutability:** Once submitted, a supervision log cannot be edited. Corrections require a new entry.
...
- **SupervisionLog**: Tracks site visits, online video meetings, or phone supervisions conducted by school teachers, storing dates, types, and private notes.

---

## Error Handling & Failure Modes

- **Unauthorized Logging:** Attempting to write a supervision log for a student who is not assigned to the teacher (via the registration mentor mappings) throws a `403 Forbidden` response.
- **Editing Supervision Logs:** Patching or updating a finalized supervision log throws a `RejectedException`.
