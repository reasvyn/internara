...
Manages system-wide configuration as a key-value store with type enforcement, caching, and dynamic
branding support. Provides the global `setting()`, `brand()`, and `app_info()` helper functions used
across all modules.
...
- **Form Objects**:
    ...
    - `BrandingForm` — 4 color inputs with hex validation, 6 preset swatches, logo/favicon upload
      with preview, remove confirmation
...
Settings cache is pre-warmed by the `system:cache-warm` command (scheduled hourly in
`routes/console.php`), which calls `Settings::all()` and `Settings::group(...)` for each group to
populate the cache.
...
- `colors.presets` — 6 named presets: Sky, Emerald, Violet, Rose, Ocean, Slate (each with label + 5
  color values)
...
- **Business Logic (`app/`)**: Located in
  [app/Settings/](file:///home/reasnovynt/Projects/Dev/reasvyn/internara/app/Settings/)
- **Routing (`routes/`)**:
  [routes/web/settings.php](file:///home/reasnovynt/Projects/Dev/reasvyn/internara/routes/web/settings.php)
- **Views (`views/`)**: Blade templates and layouts are in
  [resources/views/settings/](file:///home/reasnovynt/Projects/Dev/reasvyn/internara/resources/views/settings/)
