...
Templates are rendered using Laravel Blade with DomPDF (`DocumentRenderer`). The pipeline: load
template → substitute variables → compile Blade → render PDF. Variables resolve from the
registration context (student name, program, dates) and system settings (school name, principal
name). Rendering failures throw a `RenderException` logged via SmartLogger.
