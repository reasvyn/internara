...
For a complete walkthrough from zero to production, follow these documents
in order:

```
getting-started.md ──→ installation.md ──→ setup-wizard.md ──→ post-setup.md
      (clone to         (deployment           (7-step             (first admin
       wizard ready)     deep-dive)            wizard guide)       actions)
```

The blueprints below serve as detailed references for each topic.

## Reading Order

1. Start with [Getting Started](../getting-started.md) — end-to-end walkthrough.
2. Use this **Blueprint 01** to verify your environment meets requirements.
3. Follow **Blueprint 03** to configure your `.env` and database connection.
4. Run migrations, then open your browser to start the [Setup Wizard](../setup-wizard.md).
5. Use **Blueprint 02** (VPS) or **Blueprint 11** (shared hosting) when deploying to production.
6. Refer to **Blueprint 04** when choosing or migrating between database engines.
...
- `docs/architecture.md` — 12-layer architecture that these blueprints support
- `docs/getting-started.md` — end-to-end walkthrough
- `docs/installation.md` — deployment reference
- `docs/infrastructure.md` — deployment overview
