...
# If using FTP:
# Download .env.example, rename to .env, edit with a text editor, upload
```
...
`BROADCAST_CONNECTION=log`. Full-featured alternatives (Redis, Reverb,
database driver) are commented out with explanations.
...
The `.env.example` file defaults are already optimized for shared hosting:
...
The alternative values (database, redis, reverb) are commented out with
explanations — uncomment them when upgrading to a VPS or Docker setup.
...
- `docs/adr/adr-017-shared-hosting-deployment.md` — architecture decision
- `.env.example` — environment template with shared hosting defaults
- `routes/web/cron.php` — web-based scheduler trigger
- `docs/blueprints/02-server-deployment.md` — full VPS setup guide
