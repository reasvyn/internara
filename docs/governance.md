...
- **Authority**: The "Owner" module of a data entity is the **ONLY authority** for its existence and
  state. Never query a foreign module's model directly; utilize the designated Service Contract.
- **Protocol**: The "Parent" module's Service Contract is the authoritative source for validating
  the existence of a foreign entity.

### 4.2 Integrity Verification Patterns

#### 4.2.1 Creation & Assignment Verification (The PEP Check)
...
#### 4.2.2 Deletion & Restriction Protocols (Event-Driven)
...
### 4.3 Pattern 3: Handling Orphaned Records (Graceful Degradation)

Despite best efforts, software-level integrity can occasionally drift. Internara employs a
"Defensive Querying" strategy.

- **Graceful Degradation**: When retrieving a list of records with foreign references, the Service
  Layer should be prepared for "Missing Anchors."
- **Pattern**: If a referenced UUID no longer exists, the UI should render a "Redacted" or "Deleted
  Entity" placeholder instead of crashing with a `NullPointerException`.

### 4.4 Transactional Orchestration
...
### 4.6 Verification & Audit

Compliance with SLRI is verified through:

1.  **Architecture Police**: `tests/Arch/` ensures no physical FKs are declared in migrations
    targeting other module namespaces.
2.  **Integrity Tests**: Every service method that accepts a foreign UUID MUST have a test case for
    the "Invalid Reference" scenario.
