# Journal — Logbooks, Attendance, Absence & Monitoring Visits
...
Journal is the operational hub for daily internship activities. Students record daily logbook
...
available for corrections. Duration calculations power compliance monitoring and attendance report.
...
notification. At N+2 days, the program coordinator is also notified. The `journal:check-compliance`
...
- Report (attendance, logbook compliance, supervision log, and visit compliance data for grade card)
...
- **Compliance monitoring is proactive, not reactive** — the `journal:check-compliance` command runs on a schedule and sends escalating notify (mentor at N days → coordinator at N+2 days). Students who fall behind receive no silent pass; the system surfaces the gap, not the teacher.
