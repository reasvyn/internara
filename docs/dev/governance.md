...
- **Authority**: The "Owner" module of a data entity is the **ONLY authority** for its existence 
  and state. Never query a foreign module's model directly; utilize the designated Service Contract.
...
- **Graceful Degradation**: When retrieving a list of records with foreign references, the 
  Service Layer should be prepared for "Missing Anchors."
...
1.  **Architecture Police**: `tests/Arch/` ensures no physical FKs are declared in 
