...
1.  **Metadata SSoT**: The file `app_info.json` is the **Single Source of Truth**. Any update to the
         application's metadata (version, Blueprint ID, etc.) must be done within this file.2.  **Constant Integrity**: The constant `AUTHOR_IDENTITY` within the code is the authoritative
