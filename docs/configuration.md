...
**Static configuration** that defines how the application works at the code level. These are
**infrastructure settings** that rarely change.
...
**Dynamic configuration** stored in database (`settings` table) that can be changed by users/admins
at runtime. These are **business settings** that affect app behavior.
...
**Note**: `config()` is **not cached** during development (always reads from files). In production,
