...
## 10. PiiMasker Reference

`app/Core/Support/PiiMasker.php` — `final class PiiMasker`, 188 lines.

### Full Mask Keys (29 entries)

These keys are replaced with `'***'`. Matching uses `str_contains()` on the lowercased key name, so a key like `old_password` or `api_token` triggers a match.

| # | Key | Match Behavior |
|---|-----|----------------|
| 1 | `password` | Substring — `old_password`, `new_password`, `password_confirmation` all match |
| 2 | `password_confirmation` | Substring match |
| 3 | `current_password` | Substring match |
| 4 | `old_password` | Substring match |
| 5 | `secret` | Substring — `client_secret`, `secret_key` all match |
| 6 | `token` | Substring — `api_token`, `access_token`, `refresh_token` all match |
| 7 | `api_key` | Substring match |
| 8 | `api_token` | Substring match |
| 9 | `access_token` | Substring match |
| 10 | `refresh_token` | Substring match |
| 11 | `authorization` | Substring match |
| 12 | `credit_card` | Substring match |
| 13 | `card_number` | Substring match |
| 14 | `cvv` | Substring match |
| 15 | `ssn` | Substring match |
| 16 | `national_id` | Substring match |
| 17 | `birth_date` | Substring match |
| 18 | `dob` | Substring match |
| 19 | `address` | Substring match |
| 20 | `passport` | Substring match |
| 21 | `driver_license` | Substring match |
| 22 | `drivers_license` | Substring match |
| 23 | `security_question` | Substring match |
| 24 | `security_answer` | Substring match |
| 25 | `bank_account` | Substring match |
| 26 | `account_number` | Substring match |
| 27 | `routing_number` | Substring match |
| 28 | `health_insurance` | Substring match |
| 29 | `medical_record` | Substring match |

### Partial Mask Keys (3 entries)

These keys are matched exactly (not substring):

| Key | Mask Method | Input → Output |
|-----|------------|----------------|
| `email` | `maskEmail()` | `john.doe@example.com` → `jo***@example.com` |
| `phone` | `maskPhone()` | `081234567890` → `*******7890` |
| `name` | `maskName()` | `John Smith` → `J. Smith` / Single word `John` → `J***` |
...
### User-Agent Masking (`maskUserAgent()`)

Truncates to first 50 characters. Null/empty returns unchanged.

```
Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36...
```

### Usage Outside SmartLogger

`PiiMasker` is also used by `HasExceptionContext` trait (Core exceptions):
