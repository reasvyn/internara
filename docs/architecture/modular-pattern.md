...
Direct imports for no-side-effect access, Action delegation for cross-module operations, events for fire-and-forget, core contracts for broad abstractions. See: `docs/adr/adr-cross-module-communication.md`.

### 1.5 Gradual Migration Path
Three-phase migration: `execute(array $data)` → `execute(Data|array $data)` → `execute(Data $data)`. See: `docs/adr/adr-gradual-migration.md`.
...
**Bridge Pattern** — `Entity::fromModel(Model)` and `Model::as{Entity}(): Entity` connect persistence to business rules. **Entity Purity** — `final readonly`, zero I/O, zero persistence. **Shared Validation Rules** — entities expose static `rules()` for Form Objects. See: `docs/adr/adr-entity-model-separation.md`.
...
**Centralized Key Registry** — all keys in `config/cache-keys.php`. **Event-Driven Invalidation** — Command → event → listener → `Cache::forget()`. **TTL Categorization** — Short (<5m), Medium (5m-1h), Long (1h-24h), Forever. See: `docs/infrastructure/cache.md`.
...
**Static Utilities** — `Color`, `PiiMasker`, `CsvHandler`, `PasswordRules`, `Environment`, `AppInfo`, `AppIntegrity`, `LangChecker`. **Livewire Concerns** — `WithRecordSelection`, `WithSorting`. **CustomDatabaseChannel** — in-app DB notifications. **Security Middleware** — `SecurityHeaders` (CSP, X-Frame-Options, etc.), `LogContext` (request tracing).
