...
To ensure the delivery of high-fidelity, ISO-compliant software, Internara enforces a 
**Standardized Life Cycle Process (SLCP)**.
...
- **Technical Audit**: Perform a deep-dive into the existing codebase using search and discovery 
  tools. Identify potential impacts on existing modules and detect any "Architectural Debt" 
...
- **Blueprint Refinement**: Update or create an **Application Blueprint** (`docs/developers/blueprints/`). 
  This serves as the "Contract of Intent" and must define modular boundaries and data schema.
...
- **Functional Testing**: Write behavioral test suites (Pest v4) that mirror the source 
  structure 1:1 (**Mirroring Invariant**). Every test should refer to a Requirement ID.
- **Failure Verification**: Run `composer test` and confirm they fail. This proves the 
  requirement is not yet met and the tests are correctly identifying the gap.
...
- **Secure (S1)**: Sanitize all inputs at the boundary. Enforce **RBAC** via Policies. Use 
  **UUID v4** for identity. Mask PII in logs.
...
- **Scalable (S3)**: Centralize logic in the **Service Layer** (`EloquentQuery`). Maintain 
  **Strict Modular Isolation** by communicating exclusively via **Service Contracts**.
...
- **Static Analysis & Linting**: Execute `composer lint`. This runs **Pint** (PHP) and 
  **Prettier** (Blade/JS). Ensure zero violations.
...
- **Release Narration**: Document the changes, fixed bugs, and remaining debt in the 
  active **Release Note** (e.g., `docs/pubs/releases/`).
- **Knowledge Update**: If new protocols are introduced, update the **Developer Wiki** 
  or technical guides.
...
- **Conventional Commits**: Use the `type(scope): description` pattern (e.g., `feat(user): add 
  uuid identity`). Link to issues if applicable.
...
- **PR Readiness**: Prepare the Pull Request with a clear description and traceability to 
...
1.  **Environment Awareness**: Use `is_debug_mode()`, `is_development()`, or `is_testing()` 
    guards for environment-specific logic.
...
_Non-compliance with this lifecycle is considered an architectural defect. By following these 
steps, we ensure that Internara remains a high-quality, professional, and sustainable ecosystem._
