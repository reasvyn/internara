...
terminology, and the application ships with two complete language packs.

**Community contributions for additional languages are welcome.** See the
guide below for adding a new language.
...
Users can switch languages at any time using the language switcher in the
interface. Their preference persists across sessions.
...
JSON files contain short strings and UI labels. The key is the English string,
...
All enums implement the `LabelEnum` contract with a `label()` method. Labels
...
- Date formatting uses `Carbon` with the active locale
...
Community contributions for additional languages are welcome. To add a new
language:
...
Translate the **values** in each file. Keep the **keys** unchanged — they
must match the English version exactly.

Example — translating `lang/fr/attendance.php`:
...
- `app/Domain/Shared/Livewire/LanguageSwitcher.php` — UI language toggle
