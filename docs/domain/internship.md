...
groups, phases, and the full program lifecycle from DRAFT through COMPLETED or CANCELLED.
...
Programs flow through DRAFT → PUBLISHED → ACTIVE → COMPLETED (or CANCELLED at any stage).
Each transition has explicit preconditions. Both COMPLETED and CANCELLED are terminal states
with zero valid transitions.
...
- Manage program status through a five-stage lifecycle: draft, published, active, completed, or cancelled.
...
- Verify closure readiness by checking that all assessments are finalized, submissions graded or verified, supervision logs verified, and attendance records verified.
...
- Filter programs by status using a dropdown selector for draft, published, active, completed, and cancelled states.
