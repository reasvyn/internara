...
Provides foundational infrastructure, base classes, contracts, and application-wide utilities that every other domain depends on.
...
Located in `app/Domain/Core/Contracts/`:

| Contract | Purpose | Implemented By |
|---|---|---|
| `LabelEnum` | Provides `label(): string` for UI display | All enums across all domains |
...
Located in `app/Domain/Core/Exceptions/`. Dual hierarchy, both using `HasExceptionContext` trait:
...
Core provides 2 middleware in `app/Domain/Core/Http/Middleware/`:
...
Additional middleware used globally (located in other domains):
- `RequireSetupAccessMiddleware` — `app/Domain/Academics/Http/Middleware/RequireSetupAccessMiddleware.php` — redirects to /setup when not installed
- `SetLocaleMiddleware` — `app/Domain/SysAdmin/Aggregates/Setting/Http/Middleware/SetLocaleMiddleware.php` — language preference from session/database
...
| File | Class | Extends | Purpose |
|---|---|---|---|
| `Models/BaseModel.php` | `BaseModel` | `Model` (abstract) | Base for all domain models — UUID PKs, HasFactory, soft-delete |
...
Located in `app/Domain/Core/Support/` (13 files):
...
│   ├── DomainException.php                ← Abstract root for domain invariants
...
│   ├── RejectedException.php              ← Domain invariant violation
