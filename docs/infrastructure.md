...
This document provides a comprehensive overview of the technical stack, infrastructure requirements,
and dependencies for the `internara` project.
...
The project utilizes a modern Laravel stack focused on developer velocity and a "No-JS"
(Livewire-heavy) approach:
...
- **Purpose**: Provides elegant flash messages (success, error, warning, info) with toastr-style
  notifications
- **Theme**: Emerald (configured in `config/flasher.php`)
- **Integration**:
    - Backend: `flash()` helper function
    - Frontend: `@flasher_render` Blade directive in
