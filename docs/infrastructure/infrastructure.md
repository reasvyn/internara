...
Three background processes must be running in production (Tier 2+). In Tier 1, the queue runs synchronously, and the scheduler is triggered via HTTP webhook.
...
- [ ] Queue workers running (Supervisor with dual pipelines)
