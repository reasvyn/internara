...
> **Last updated:** 2026-07-31 **Changes:** feat — new spec for the consolidated module gateway
> (ModuleManager + ModuleService) that supersedes the ModuleDiscoverService API documented in
> `module-discovery.md` (#5)
...
- Config structure: `config/module.php` — see `module-discovery.md` (#5) §6.1 (unchanged; FR-MIG6).
- Cache keys: `config/cache-keys.php` keys `module_livewire`, `module_policies`, `module_views`,
  TTL 86400 — see `module-discovery.md` (#5) §6.5 (unchanged).
...
**Rationale:** Consistent with `module-discovery.md` (#5) NG4 and DD-1. Runtime mutability adds
...
|------|------------------|
| [base-classes.md](base-classes.md) (#2) | Class conventions (Support static / Service instance) and test patterns |
| [shared-utilities.md](shared-utilities.md) (#3) | `SmartLogger` used by `ModuleDiscoverCommand` for discovery logging |
| [module-discovery.md](module-discovery.md) (#5) | `config/module.php` registry, `config/cache-keys.php` (`module_livewire`, `module_policies`, `module_views`), discovery baseline and conventions |
...
|-------|------|------------|
| 1 | [installation.md](installation.md) (#12) | Setup/installation flows verify module registration and rely on the consolidated gateway (`module:discover`) |
...
- `docs/specs/module-discovery.md` (#5) — Registry, discovery conventions, config contract
